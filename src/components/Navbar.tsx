...
const scrollTo = (href: string, afterClose?: () => void) => {
  const run = () => {
    if (href === "#") {
  ...
  if (afterClose) {
    afterClose();
    // Wait for mobile menu close animation (300ms) then scroll
    setTimeout(run, 320);
  } else {
    run();
  }
  ...
  return (
    <>
      <style>{`
        ...
        .nav-link-hover:hover::before { opacity: 1; top: -14px; }
      `}</style>
            ...
            >
              <div className="max-w-6xl mx-auto px-6 py-4 space-y-1">
                ...
                <div className="pt-3 border-t border-border/30">
                  <button
                    onClick={() =>
                      scrollTo("#contact", () => setMobileOpen(false))
                    }
